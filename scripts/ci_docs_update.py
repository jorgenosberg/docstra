...
Usage:
    python ci_docs_update.py --codebase /path/to/repo --output /path/to/docs
    python ci_docs_update.py --help
"""
...
def run_docstra_command(cmd: List[str], cwd: str) -> bool:
    ...
    try:
        ...
        logger.error("docstra command not found. Please ensure docstra is installed.")
        return False


def check_documentation_status(codebase_path: str, config_path: Optional[str] = None) -> Dict:
    """Check the current documentation status."""
    logger = setup_logging()
    
    cmd = ["docstra", "status", codebase_path, "--detailed"]
    if config_path:
...
def update_documentation(codebase_path: str, 
                        output_dir: str, 
                        base_ref: str,
                        config_path: Optional[str] = None,
                        force_files: Optional[List[str]] = None) -> bool:
    """Update documentation incrementally."""
    logger = setup_logging()
    
    cmd = ["docstra", "update", codebase_path, "--base", base_ref, "--output", output_dir]
...
def generate_full_documentation(codebase_path: str, 
                               output_dir: str,
                               config_path: Optional[str] = None) -> bool:
    """Generate full documentation (fallback when incremental fails)."""
    logger = setup_logging()
    
    cmd = ["docstra", "generate", codebase_path, "--output", output_dir, "--no-wizard"]
...
def create_ci_summary(codebase_path: str, 
                     output_dir: str, 
                     success: bool, 
                     ci_info: Dict[str, Any],
...
def main():
    ...
    parser.add_argument(
        "--config",
        help="Path to docstra configuration file"
    )
    ...
    parser.add_argument(
        "--force-files",
        nargs="*",
        help="Force regeneration for specific files"
    )
    ...
    logger.info(f"Starting CI/CD documentation update")
    ...
    try:
        ...
        if not args.skip_status_check:
            ...
            if not status_result["success"]:
                logger.warning("Could not check documentation status, proceeding anyway")
        ...
        if args.full_generation:
            ...
            if not success:
                logger.warning("Incremental update failed, falling back to full generation")
                success = generate_full_documentation(
                    str(codebase_path), 
                    str(output_dir), 
                    args.config
        ...
        if success:
            logger.info(f"Documentation update completed successfully in {execution_time:.2f} seconds")
            print(f"::notice::Documentation updated successfully in {execution_time:.2f}s")
        else:
