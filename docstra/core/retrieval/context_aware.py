...
class ContextAwareRetriever:
    ...
    def _analyze_query(self, query: str) -> QueryAnalysis:
        ...
        if any(re.search(pattern, query_lower) for pattern in architectural_patterns):
            ...
            re.search(pattern, query_lower) for pattern in implementation_patterns
        ):
        ...
        return QueryAnalysis(
            intent=intent, symbols=symbols, files=files, complexity=complexity
        )
    ...
    def _get_architectural_context(
        ...
        if self.repo_map and remaining_budget > 500:
            ...
            if repo_overview:
                overview_tokens = self.budget_manager.token_counter.count_tokens(
                    repo_overview
                )
                # Use more budget for overview in detailed mode
                max_overview_budget = int(
                    remaining_budget * (0.2 if is_detailed_mode else 0.3)
                )
        ...
        if remaining_budget > 300:
            module_budget = int(
                budget * (0.4 if is_detailed_mode else 0.3)
            )  # Use original budget
            modules_context = self._get_relevant_modules_context(
                analysis, module_budget
            )
            if modules_context:
                context_parts["relevant_modules"] = modules_context
                module_tokens = self.budget_manager.token_counter.count_tokens(
                    modules_context
                )
        ...
        if remaining_budget > 200:
            ...
            samples_budget = min(
                remaining_budget,
                int(budget * (0.3 if is_detailed_mode else 0.2)),
            )
            code_samples = self._get_targeted_code_samples(
                query, analysis, samples_budget
            )
            if code_samples:
                context_parts["code_samples"] = code_samples
                samples_tokens = self.budget_manager.token_counter.count_tokens(
                    code_samples
                )
    ...
    def _get_implementation_context(
        ...
        if analysis.symbols and remaining_budget > 400:
            impl_context = self._get_symbol_implementations(
                analysis.symbols, int(remaining_budget * 0.5)
            )
            if impl_context:
                context_parts["implementations"] = impl_context
                impl_tokens = self.budget_manager.token_counter.count_tokens(
                    impl_context
                )
        ...
        if remaining_budget > 300:
            examples = self._get_code_examples(
                query, analysis, int(remaining_budget * 0.6)
            )
            if examples:
                context_parts["examples"] = examples
                example_tokens = self.budget_manager.token_counter.count_tokens(
                    examples
                )
        ...
        if remaining_budget > 200:
            arch_context = self._get_minimal_architectural_context(
    ...
    def _get_usage_context(
        ...
        if remaining_budget > 400:
            ...
            if usage_examples:
                ...
                usage_tokens = self.budget_manager.token_counter.count_tokens(
                    usage_examples
                )
        ...
        # 2. API documentation
        if remaining_budget > 200:
            api_context = self._get_api_context(analysis, int(remaining_budget * 0.4))
            if api_context:
    ...
    def _get_general_context(
        ...
        if results:
            ...
            assembled_context = self._assemble_results_with_budget(
                prioritized_results, remaining_budget
            )

            if assembled_context:
                context_parts["search_results"] = assembled_context
                result_tokens = self.budget_manager.token_counter.count_tokens(
                    assembled_context
                )
    ...
    def _get_relevant_modules_context(
        self, analysis: QueryAnalysis, budget: int
    ) -> Optional[str]:
            ...
            for file_path, metadata in self.code_index.file_index.items():
                ...
                for func in functions:
                    if any(
                        ...
                        for keyword in ["command", "cli", "main", "app"]
                    ):
                ...
                if metadata:
                    ...
                    if "cli.py" in module_path.lower() and len(functions) > 0:
                        ...
                        if is_detailed:
                            ...
                            import_summary = (
                                f", imports from {len(imports)} modules"
                                if imports
                                else ""
                            )
        ...
        for symbol in analysis.symbols[:2]:  # Limit to avoid budget overflow
            if self.code_index:
                ...
                for loc in symbol_locs[:1]:  # One location per symbol
                    ...
                    if related_files:
                        deps_info.append(
                            f"- {symbol} (in {filepath}) relates to: {', '.join(related_files[:3])}"
                        )
    ...
    def _get_symbol_implementations(
        self, symbols: List[str], budget: int
    ) -> Optional[str]:
        ...
        for symbol in symbols:
            ...
            for loc in locations[:1]:  # One implementation per symbol
                ...
                try:
                    ...
                    tokens_needed = self.budget_manager.token_counter.count_tokens(
                        impl_context
                    )
    ...
    def _get_minimal_architectural_context(
        self, analysis: QueryAnalysis, budget: int
    ) -> Optional[str]:
    ...
    def _prioritize_results(
        self, results: List[Dict[str, Any]], analysis: QueryAnalysis
    ) -> List[Dict[str, Any]]:
        ...
        return sorted(
            prioritized, key=lambda x: x.get("priority_score", 0), reverse=True
        )

    def _assemble_results_with_budget(
        self, results: List[Dict[str, Any]], budget: int
    ) -> Optional[str]:
        ...
        for file_path in key_files[:2]:  # Limit to 2 files to avoid budget overflow
            try:
                ...
                if metadata:
                    ...
                    file_content = "\n".join(file_summary)
                    content_tokens = self.budget_manager.token_counter.count_tokens(
                        file_content
                    )
