...
class RepositoryExplorerService:
    ...
    def _load_components(self, codebase_path: str) -> None:
        ...
        if not self.repo_map or not self.code_index:
            raise ValueError("Repository not fully indexed. Run 'docstra ingest' first.")
    ...
    def get_file_relationships(self, file_path: str) -> Dict[str, Any]:
        ...
        symbols = []
        if file_metadata:
            symbols = file_metadata.get("functions", []) + file_metadata.get("classes", [])
    ...
    def explore_structure(self, path: str, depth: int = 3, show_tree: bool = False) -> Dict[str, Any]:
        ...
        def build_node_tree(node, current_depth: int = 0) -> Dict[str, Any]:
            ...
            if hasattr(node, 'children'):  # DirectoryNode
                ...
                for name, child in node.children.items():
                    node_data["children"][name] = build_node_tree(child, current_depth + 1)
    ...
    def _build_flat_structure(self, depth: int) -> Dict[str, Any]:
        ...
        def traverse_node(node, current_depth: int = 0, path_parts: Optional[List[str]] = None):
    ...
    def display_file_relationships(self, relationships: Dict[str, Any]) -> None:
        ...
        if relationships["dependencies"]:
            deps_table = Table(title="Dependencies", show_header=True, header_style="bold cyan")
        ...
        if relationships["dependents"]:
            deps_table = Table(title="Dependents", show_header=True, header_style="bold green")
        ...
        if relationships["symbols"]:
            symbols_table = Table(title="Symbols", show_header=True, header_style="bold magenta")
            ...
            if len(relationships["symbols"]) > 10:
                symbols_table.add_row(f"... and {len(relationships['symbols']) - 10} more")
        ...
        if complexity_info:
            ...
            info_text += f"[{Colors.BOLD}]Size:[/] {complexity_info.get('size_kb', 0):.1f} KB\n"
