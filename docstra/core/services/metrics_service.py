...
class MetricsService:
    ...
    def _analyze_dependencies(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "highly_coupled_files": sorted(
                highly_coupled, key=lambda x: x["dependency_count"], reverse=True
            )[:10],
    ...
    def _analyze_module_breakdown(self, overview: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for category, files in modules.items():
            breakdown[category] = {
                ...
                "languages": list(
                    set(f.get("language") for f in files if f.get("language"))
                ),
                "total_symbols": sum(len(f.get("symbols", [])) for f in files),
                "avg_symbols_per_file": sum(len(f.get("symbols", [])) for f in files)
                / len(files)
                if files
                else 0,
            }
    ...
    def detect_dependency_cycles(self) -> List[List[str]]:
        ...
        def dfs_detect_cycle(node: str, path: List[str]) -> None:
            ...
            for dependency in dependencies.get(node, []):
                ...
                ):  # Only follow if dependency is also in our map
    ...
    def _get_most_stable_files(
        self, instability: Dict[str, float]
    ) -> List[Dict[str, Any]]:
    ...
    def _get_most_unstable_files(
        ...
        """
        sorted_files = sorted(instability.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
        return [
            {"file": os.path.basename(file_path), "instability": score}
    ...
    def generate_quality_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                complexity_analysis, dependency_analysis, coupling_analysis
            ),
        }

    def _calculate_quality_score(
        self, complexity_analysis: Dict[str, Any], dependency_analysis: Dict[str, Any]
    ) -> float:
        ...
        high_complexity_ratio = complexity_analysis.get(
            "high_complexity_files", 0
        ) / max(1, complexity_analysis.get("total_files", 1))
    ...
    def _generate_recommendations(
        ...
        high_coupling_files = len(
            [
                f
                for f in coupling_analysis.get("efferent_coupling", {}).values()
                if f > 10
            ]
        )
        ...
        if not recommendations:
            recommendations.append(
                "Code quality metrics look good! Keep up the good work."
            )
    ...
    def display_repository_overview(self, overview: Dict[str, Any]) -> None:
        ...
        stats_table = Table(
            title="Repository Statistics", show_header=True, header_style="bold cyan"
        )
        ...
        if repo_stats.get("languages"):
            ...
            for language, count in sorted(
                repo_stats["languages"].items(), key=lambda x: x[1], reverse=True
            ):
    ...
    def display_dependency_cycles(self, cycles: List[List[str]]) -> None:
        ...
        if len(cycles) > 5:
            self.console.print(
                f"[{Colors.DIM}]... and {len(cycles) - 5} more cycles[/]"
            )
