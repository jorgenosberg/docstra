...
class VisualizationService:
    ...
    def generate_complexity_heatmap(self, output_file: Optional[str] = None) -> str:
        ...
        try:
            ...
            sorted_files = sorted(
                complexity_data.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def generate_architecture_diagram(self, output_file: Optional[str] = None) -> str:
        ...
        try:
            ...
            for category, files in modules.items():
                ...
                files_with_symbols = [
                    (f.get("path", ""), len(f.get("symbols", []))) for f in files
                ]
                top_files = sorted(
                    files_with_symbols, key=lambda x: x[1], reverse=True
                )[:3]
                ...
                if top_files:
                    ...
                    for file_path, symbol_count in top_files:
                        if file_path:
                            arch_content.append(
                                f"  - {os.path.basename(file_path)} ({symbol_count} symbols)"
                            )
...
"""
                ...
                if len(files) > 5:
                    html_content += (
                        f"<li><em>... and {len(files) - 5} more files</em></li>"
                    )
